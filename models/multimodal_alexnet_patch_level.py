...
Description:
    ...
    persistence from the newer version of this script.
...
Usage:
    ...
    #   --mode   = train
    python multimodal_alexnet_patch_level.py

    # ── Auto-split from a single root folder with augmentation ──
    python multimodal_alexnet_patch_level.py --data data/ --split 0.7 0.2 0.1 --mode train --augment

    # 80% train / 20% val, no test split
    python multimodal_alexnet_patch_level.py --data data/ --split 0.8 0.2 --mode train

    # 100% goes to the chosen mode — useful for standalone eval after training
    python multimodal_alexnet_patch_level.py --data data/ --split 1.0 --mode test

    # ── Pre-split folders ──
    python multimodal_alexnet_patch_level.py --mode train --train_dir data/train --augment
    python multimodal_alexnet_patch_level.py --mode train --train_dir data/train --val_dir data/val
    python multimodal_alexnet_patch_level.py --mode val   --val_dir  data/val
    python multimodal_alexnet_patch_level.py --mode test  --test_dir data/test
...
class MultimodalDataset(Dataset):
    ...
    def __init__(
        ...
        for class_name in class_names:
            ...
            class_dir = os.path.join(image_dir, class_name)  # FIX: was built from an undefined 'class_dir'
...
# ===============================================================
# Model Definition (AlexNet + Clinical Branch)
# ===============================================================
class MultimodalAlexNet(nn.Module):
    """
    AlexNet image backbone optionally fused with a clinical/morphometric branch.
    ...
    # AlexNet's penultimate FC layer always outputs 4096 features.
    _ALEXNET_FEATURE_DIM = 4096

    def __init__(self, clinical_input_dim: int, num_classes: int = 21):
        super().__init__()

        # Load AlexNet with up-to-date weights API (pretrained=True is deprecated)
        backbone = alexnet(weights=AlexNet_Weights.DEFAULT)

        # Drop the final classification layer so we get raw 4096-d embeddings.
        ...
        if self.use_clinical:
            ...
            fusion_dim = self._ALEXNET_FEATURE_DIM + 32
        else:
            self.clinical_net = None
            fusion_dim = self._ALEXNET_FEATURE_DIM
...
def load_model(
    ...
    model = MultimodalAlexNet(clinical_input_dim, num_classes=num_classes)
...
def get_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Multimodal AlexNet for image-patch classification (defaults to the UCMerced Land Use dataset)",
